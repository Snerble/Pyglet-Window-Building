...
from typing import Any, Callable, List, Union, Mapping
...
class Component:
    ...
    def width(self) -> Union[int, float]:
        # Return the cached value or recalculate the cache
        if issubclass(type(self.__bounds[2]), Constraint):
            width = self.__get_cache("width", self.__bounds[2])
        else:
            width = self.__bounds[2]
        
        max_width = self.max_width
        return min(max_width, width) if max_width else width
    @width.setter
    def width(self, value: Union[str, int, float, Constraint]):
        ...
        if type(value) == str:
            if value == "match_parent":
                ...
                self.set_value_watcher(self.parent, "width", "max_content_width")
                remove_watcher = False
            else:
                # TODO parse special value units
                value = float(value)
        
        assert type(value) in (int, float, str) and value >= 0 or issubclass(type(value), Constraint)

        # Clear this value's cache
        self.__cache.pop("width", None)
        if remove_watcher: self.remove_value_watcher(self.parent, "width", "max_content_width")

        self.__bounds[2] = value

        # TODO invalidate instead of notifying the watchers here (max_content_width may or may not be cached)
        # Notify the watchers on related values
        self.notify_value_watchers("max_content_width")
    @width.deleter
    def width(self):
        self.__cache.pop("width", None)
        self.notify_value_watchers("max_content_width")
    ...
    def max_width(self) -> Union[float, None]:
        ...
        if issubclass(type(self.__max_size[0]), Constraint):
            return self.__get_cache("max_width", self.__max_size[0])
    ...
    def max_width(self, value: Union[str, int, float, Constraint]):
        ...
        if type(value) == str:
            if value == "match_parent":
                ...
                self.set_value_watcher(self.parent, "max_width", "max_content_width")
                remove_watcher = False
            else:
                value = float(value)
        ...
        # Clear this value's cache
        self.__cache.pop("max_width", None)
        if remove_watcher: self.remove_value_watcher(self.parent, "max_width", "max_content_width")

        self.__max_size[0] = value
        
        # Notify value watchers for related values
        self.notify_value_watchers("width")
        self.notify_value_watchers("max_content_width")
    @max_width.deleter
    def max_width(self):
        self.__cache.pop("max_width", None)
