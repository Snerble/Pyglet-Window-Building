...
class Program:
	def __init__(self, caption=None, width=1280, height=720, **kwargs: dict):
		"""
		...
		self.window = window.Window(width, height, config=_config, **kwargs)
...
if __name__ == "__main__":
	...
	program = Program("E621 Browser",
		...
		height=int(720*1.25),
		# resizable=False,
		# style=pyglet.window.Window.WINDOW_STYLE_BORDERLES
	)
	...
	program.window.set_location((screen.width - program.window.width) // 2, (screen.height - program.window.height) // 2)
